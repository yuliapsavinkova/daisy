...
(function () {
  ...
  // Single glob — guarantees all images are emitted into the production build
  ...
  urls.forEach((url, i) => {
    ...
    img.width = 900; // prevents CLS while image loads
    img.height = 595;

    if (i === 0) {
      // First slide is the LCP candidate — fetch immediately at high priority
      img.loading = 'eager';
      img.fetchPriority = 'high';
      img.decoding = 'sync';
    } else {
      // Off-screen slides — defer until browser has spare bandwidth
      img.loading = 'lazy';
      img.decoding = 'async';
    }
  ...
  // Clones are always off-screen at mount
